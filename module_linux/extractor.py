...
def check(words):
    if(searchtag(words,"patent application publication")==Pair(-1, -1)):
        return False;
    else:
        return True;
            ...
            if(words[i+1].lower()=="of"):
                if(words[i+2].lower()=="pages"):
                    if(i+4>=len(words)):
                        ...
                        flag[pages_tag]=1
                        pages = Pair(i,i+3)          
                    elif(bool(re.search(r'\D',words[i+4]))):  #if match found it must assign it words[i+4] not "NA"
                        data[pages_tag] = "NA"
                        flag[pages_tag]=1
                        pages = Pair(i,i+3)
                    else:
                        data[pages_tag] = words[i+4]
                        flag[pages_tag]=1
                        pages = Pair(i,i+4)
                    ...
                    if(i+4>=len(words)):
                        ...
                        flag[claims_tag]=1
                        claims = Pair(i,i+3)
                    elif(bool(re.search(r'\D',words[i+4]))):
                        data[claims_tag] = "NA"
                        flag[claims_tag]=1
                        claims = Pair(i,i+3)
                    else:
                        data[claims_tag] = words[i+4]
                        flag[claims_tag]=1
                        claims = Pair(i,i+4)
...
#       myi+=1       
      ...
      while p < len(indexvalues)-1:
          if(indexvalues[p].tag!="(19) India" and flag[indexvalues[p].tag]!=1):
  ...
  #            print(indexvalues[p+1].start)
    ...
    #  if(flag["(57) Abstract"]!=1): 
      ...
      if(data["(87) International Publication"]=="NA"):        #assuming WIPO=International publication
       data["(87) International Publication"]=data["(87) WIPO"] 
     #if(flag["(21) Application"]!=1):        
